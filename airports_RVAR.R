...
# Filter and select observations without NAs, 2020 = 5601, 2019 = 5235
...
for (clust in 1:nClusters) {
  ...
  for (i in 1:length(kList)) {
    start_value=start_recipr(vario=Gam_emtp2_list[[clust]],graph = graph_coloring,partition = partiList[[i]])
    res=Scoring_alg_col_recipr(Qhat=Theta2Q(Gamma2Theta(Gam_emtp2_list[[clust]])),graph = graph_coloring,partition = partiList[[i]],start_recipr=start_value,verbose = FALSE,tol=1e-6)
